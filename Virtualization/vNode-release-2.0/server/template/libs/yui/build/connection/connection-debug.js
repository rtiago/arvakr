...
YAHOO.util.Connect =
{
   ...
   * @property _has_https_headers
   * @private
   * @static
   * @type boolean
   */
	_has_https_headers:false,
  ...
  */
   ...
   */
	createXhrObject:function(transactionId)
	{
		...
		try
		{
			// Instantiates XMLHttpRequest in non-IE browsers and assigns to https.
			https = new XMLHttpRequest();
			//  Object literal with https and tId properties
			obj = { conn:https, tId:transactionId };
		...
		catch(e)
		{
			for(var i=0; i<this._msxml_progid.length; ++i){
				try
				{
					// Instantiates XMLHttpRequest for IE and assign to https
					https = new ActiveXObject(this._msxml_progid[i]);
   ...
   */
	asyncRequest:function(method, uri, callback, postData)
	{
		...
		if(!o){
			...
			if(this._has_default_headers || this._has_https_headers){
				this.setHeader(o);
			}
   ...
   */
    ...
    {
		var httpsStatus, responseObject;
		var args = (callback && callback.argument)?callback.argument:null;

		try
		{
			if(o.conn.status !== undefined && o.conn.status !== 0){
				httpsStatus = o.conn.status;
			}
			else{
				httpsStatus = 13030;
			}
		...
		if(httpsStatus >= 200 && httpsStatus < 300 || httpsStatus === 1223){
			responseObject = this.createResponseObject(o, args);
			if(callback && callback.success){
				if(!callback.scope){
					callback.success(responseObject);
					YAHOO.log('Success callback. HTTP code is ' + httpsStatus, 'info', 'Connection');
				}
				else{
					// If a scope property is defined, the callback will be fired from
					// the context of the object.
					callback.success.apply(callback.scope, [responseObject]);
					YAHOO.log('Success callback with scope. HTTP code is ' + httpsStatus, 'info', 'Connection');
				}
			}
			...
			switch(httpsStatus){
				...
				case 13030: // See above comments for variable status.
					...
					if(callback && callback.failure){
						if(!callback.scope){
							...
							YAHOO.log('Failure callback. Exception detected. Status code is ' + httpsStatus, 'warn', 'Connection');
						}
						else{
							callback.failure.apply(callback.scope, [responseObject]);
							YAHOO.log('Failure callback with scope. Exception detected. Status code is ' + httpsStatus, 'warn', 'Connection');
						}
					}
				...
				default:
					...
					if(callback && callback.failure){
						if(!callback.scope){
							callback.failure(responseObject);
							YAHOO.log('Failure callback. HTTP status code is ' + httpsStatus, 'warn', 'Connection');
						}
						else{
							callback.failure.apply(callback.scope, [responseObject]);
							YAHOO.log('Failure callback with scope. HTTP status code is ' + httpsStatus, 'warn', 'Connection');
						}
					}
			}
   ...
   */
	...
	{
		var headerObj = (isDefault)?this._default_headers:this._https_headers;
   ...
   */
	setHeader:function(o)
	{
		...
		if(this._has_https_headers){
			for(var prop in this._https_headers){
				if(YAHOO.lang.hasOwnProperty(this._https_headers, prop)){
					o.conn.setRequestHeader(prop, this._https_headers[prop]);
					YAHOO.log('HTTP header ' + prop + ' set with value of ' + this._https_headers[prop], 'info', 'Connection');
				}
			}
			delete this._https_headers;

			this._https_headers = {};
			this._has_https_headers = false;
		}
	},
